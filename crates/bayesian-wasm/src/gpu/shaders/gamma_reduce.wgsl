// Gamma distribution BATCHED log_prob with REDUCTION
//
...
fn main(
    ...
    for (var i: u32 = 0u; i < ELEMS_PER_THREAD; i = i + 1u) {
        ...
        if (data_idx < params.count) {
            ...
            // Gamma log_prob: log_norm + (alpha-1)*log(x) - beta*x
            local_sum = local_sum + (params.log_norm + (alpha - 1.0) * log(x) - beta * x);
        }
    }
