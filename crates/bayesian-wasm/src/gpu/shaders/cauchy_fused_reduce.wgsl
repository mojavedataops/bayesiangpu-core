// Cauchy distribution FUSED logp + multi-grad REDUCE kernel
//
...
// grad_loc    = 2 * z / (scale * (1 + z^2))
// grad_scale  = (-1 + z^2) / (scale * (1 + z^2))
// where z = (x - loc) / scale
//
...
fn main(
    ...
    for (var i: u32 = 0u; i < ELEMS_PER_THREAD; i = i + 1u) {
        ...
        if (data_idx < params.count) {
            ...
            local_grad_loc = local_grad_loc + (2.0 * z / (scale * one_plus_zsq));
            local_grad_scale = local_grad_scale + ((-1.0 + z_sq) / (scale * one_plus_zsq));
        }
    }
