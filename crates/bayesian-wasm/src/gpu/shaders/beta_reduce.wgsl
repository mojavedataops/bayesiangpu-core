// Beta distribution BATCHED log_prob with REDUCTION
//
// Computes log_prob for N observations (0 < x < 1), then reduces to partial sums.
// Each workgroup outputs one partial sum. Final reduction done on CPU.
//
// For Beta(alpha, beta) at point 0 < x < 1:
// log_prob = lgamma(alpha + beta) - lgamma(alpha) - lgamma(beta)
...
struct Params {
    ...
    beta: f32,
    count: u32,
    // Pre-computed on CPU: lgamma(alpha + beta) - lgamma(alpha) - lgamma(beta)
    log_norm: f32,
}
...
fn main(
    ...
    for (var i: u32 = 0u; i < ELEMS_PER_THREAD; i = i + 1u) {
        ...
        if (data_idx < params.count) {
            ...
            // Beta log_prob: log_norm + (alpha-1)*log(x) + (beta-1)*log(1-x)
            local_sum = local_sum + (params.log_norm + (alpha - 1.0) * log(x) + (beta_param - 1.0) * log(1.0 - x));
        }
    }
