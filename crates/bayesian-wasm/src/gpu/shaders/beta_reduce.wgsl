...
fn main(
    ...
    if (idx < params.count) {
        ...
        // Beta log_prob: lgamma(alpha+beta) - lgamma(alpha) - lgamma(beta)
        //              + (alpha-1)*log(x) + (beta-1)*log(1-x)
        let log_beta_fn = lgamma(alpha + beta_param) - lgamma(alpha) - lgamma(beta_param);
        log_prob = log_beta_fn + (alpha - 1.0) * log(x) + (beta_param - 1.0) * log(1.0 - x);
    }
