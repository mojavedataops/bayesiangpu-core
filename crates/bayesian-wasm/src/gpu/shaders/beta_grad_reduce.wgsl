// Beta distribution gradient REDUCE kernel
//
// Computes grad_log_prob for N observations (0 < x < 1), then reduces to partial sums.
// Each workgroup outputs one partial sum. Final reduction done on CPU.
//
// For Beta(alpha, beta) at point 0 < x < 1:
// grad_log_prob = (alpha - 1) / x - (beta - 1) / (1 - x)
...
fn main(
    ...
    for (var i: u32 = 0u; i < ELEMS_PER_THREAD; i = i + 1u) {
        ...
        if (data_idx < params.count) {
            ...
            local_sum = local_sum + ((alpha - 1.0) / x - (beta_param - 1.0) / (1.0 - x));
        }
    }
