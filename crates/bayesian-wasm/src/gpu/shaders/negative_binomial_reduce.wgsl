// Negative Binomial distribution BATCHED log_prob with REDUCTION
//
...
//
// NegativeBinomial(r, p) - number of successes before r failures
// log_prob = lgamma(k + r) - lgamma(k + 1) - lgamma(r) + r*log(p) + k*log(1-p)
//
// where k = x (observed successes), r = number of failures, p = failure probability

struct Params {
    r: f32,      // number of failures (as f32 for GPU)
    p: f32,      // failure probability
...
fn main(
    ...
    for (var i: u32 = 0u; i < ELEMS_PER_THREAD; i = i + 1u) {
        ...
        if (data_idx < params.count) {
            let k = x_values[data_idx];  // observed successes
            let r = params.r;       // failures required
            let p = params.p;       // failure probability

            // Negative Binomial log probability
            // log_prob = lgamma(k + r) - lgamma(k + 1) - lgamma(r) + r*log(p) + k*log(1-p)
            // Use exact lookup for k! (observed successes, typically small)
            local_sum = local_sum + (lgamma(k + r) - log_factorial_exact(u32(k)) - lgamma(r)
                     + r * log(p) + k * log(1.0 - p));
        }
    }
