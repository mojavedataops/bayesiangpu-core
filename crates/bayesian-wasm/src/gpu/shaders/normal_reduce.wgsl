// Normal distribution BATCHED log_prob with REDUCTION
//
...
// (fast for ~4000 partial sums) or could add another kernel pass.
//
// This is the realistic MCMC pattern: compute likelihood, return scalar.
...
fn main(
    ...
    for (var i: u32 = 0u; i < ELEMS_PER_THREAD; i = i + 1u) {
        ...
        if (data_idx < params.count) {
            ...
            let z = (x - mu) / sigma;
            local_sum = local_sum + (-0.5 * LOG_2PI - log(sigma) - 0.5 * z * z);
        }
    }
